    ...
    if($_POST["submit_signup"]) {
        ...
        if(!$submitted_firstname) {
            ...
            if($row['lastname']) {
                ...
                $statement = 'INSERT INTO participants (firstname, lastname, email, affiliation, cdate, followupversion, followup) VALUES (\''.ucwords(strtolower($submitted_firstname)).'\', \''.ucwords(strtolower($submitted_lastname)).'\', \''.$submitted_email.'\', \''.$affiliation.'\', \''.$date.'\', '.$followupversion.', '.$followup.')';
                $db->exec($statement);
                $db->close(); 
