...
<?php
    ...
    if($_POST["submit_signup"]) {
        ...
        if(!$submitted_firstname) {
            ...
            $statement = 'CREATE TABLE IF NOT EXISTS participants(firstname TEXT, lastname TEXT, email TEXT, cdate TEXT)';
            $db->exec($statement);
            ...
            if($row['lastname']) {
                ...
                $statement = 'INSERT INTO participants (firstname, lastname, email, cdate) VALUES (\''.ucwords(strtolower($submitted_firstname)).'\', \''.ucwords(strtolower($submitted_lastname)).'\', \''.$submitted_email.'\', \''.$date.'\')';
                $db->exec($statement);
                $db->close();
